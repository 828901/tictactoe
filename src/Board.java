...
public class Board {
    ...
    // Author: Noah
    // Precondition: N/A
    // Postcondition: A new board objects is created and int[][] board is filled with 0s.
    public Board() {
        for(int i = 0; i < 3; i++){
            for(int x = 0; x < 3; x++){
                board[i][x] = 0;
    ...
    // Author: Noah
    // Precondition: board exists and x and y are integers between 0 an 2 (inclusive)
    ...
    // Author Harry
    // Precondition: board exists
    // Postcondition: The following is returned: 0 if no player has won; 1 if x has won; -1 if 0 has won.
